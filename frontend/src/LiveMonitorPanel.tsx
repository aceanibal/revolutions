...
export function LiveMonitorPanel({
  ...
  const subscribedSet = new Set(subscribedAssets.map((s) => s.toUpperCase()));
  ...
  return (
    ...
    >
      ...
      <div className={compact ? "flex min-h-0 flex-1 flex-col" : "mt-4"}>
        <div
          className={
            compact
              ? "flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-slate-100 bg-slate-50/60"
              : "overflow-hidden rounded-xl border border-slate-100 bg-slate-50/60"
          }
        >
          <div className={compact ? "min-h-0 flex-1 overflow-auto" : "overflow-auto"}>
                  ...
                  rows.map((row: LiveMonitorRow) => (
                    <tr key={row.coin} className="hover:bg-slate-50/80">
                      ...
                      <td className="px-3 py-2 text-right">
                        {!subscribedSet.has(row.coin.toUpperCase()) && (
                          <button
                            type="button"

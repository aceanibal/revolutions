    ...
    onEvent({ event, state }) {
      ...
      if (hhmm >= sessionEndHHMM) {
        if (runnerPosition) {
          ...
          if (debug) {
            console.log(
              `[orb-avwap-930-open-avwap-sl-1m] session_end_exit day=${dayKey} at=${etLabel} ET side=${String(runnerPosition.side || "").toLowerCase() || "long"} px=${Number(sessionEndExitPx).toFixed(6)}`
            );
          }
          position = null;
          return {

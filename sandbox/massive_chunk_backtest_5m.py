...
Outputs under sandbox/cache/:
  massive_chunk_preflight.csv
  massive_chunk_results.csv          (per symbol × chunk)
  massive_chunk_results_by_asset.csv (per symbol totals)
  optional ledgers: massive_chunk_ledger_<SYMBOL>.csv
"""
...
REPORT_MD = os.path.join(_SANDBOX, "FINAL_MASSIVE_CHUNK_BACKTEST.md")

# Paths as shown in markdown (repo-relative)
def _md_path(abs_path: str) -> str:
    repo = os.path.abspath(os.path.join(_SANDBOX, ".."))
    try:

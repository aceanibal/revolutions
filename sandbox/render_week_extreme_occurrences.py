#!/usr/bin/env python3
"""
Render candlestick charts for every weekly-extreme touch occurrence.

Input:
  - JSON report produced by week_extreme_hit_study.py

Output:
  - One folder per event with:
      - 5m_candles.png  (session window with weekly levels)
      - 1m_candles.png  (zoom around hit time)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Render week-extreme occurrence candlestick charts")
    p.add_argument("--study-json", required=True, help="week_extreme_hit_study.py JSON output")
    p.add_argument("--db", default=DB_PATH)
    p.add_argument("--out-dir", default="", help="Output root folder for images")
    p.add_argument("--pre-1m-minutes", type=int, default=90, help="1m chart minutes before hit")
    p.add_argument("--post-1m-minutes", type=int, default=120, help="1m chart minutes after hit")
    return p.parse_args()
...
def save_event(event_idx: int, event: dict[str, Any], snapshots_by_anchor: dict[int, dict[str, Any]], candles_5m: list[dict[str, Any]], candles_1m: list[dict[str, Any]], out_root: str, pre_1m_minutes: int, post_1m_minutes: int) -> None:
    ...
    # --- 5m chart: full overnight session ---
    ...
    ax5.axhline(level, color="black", linestyle="-", linewidth=1.2, label=f"{event_type} level={level:.6f}")
    if week_high is not None:
        ax5.axhline(float(week_high), color="#d35400", linestyle=":", linewidth=1.0, label=f"week_high={float(week_high):.6f}")
    if week_low is not None:
        ax5.axhline(float(week_low), color="#16a085", linestyle=":", linewidth=1.0, label=f"week_low={float(week_low):.6f}")
    ...
    ax1.axhline(level, color="black", linestyle="-", linewidth=1.2, label=f"{event_type} level={level:.6f}")
...
def main() -> int:
    ...
    if not out_root:
        out_root = os.path.join(
            ...
            "cache",
            f"week_extreme_occurrences_{symbol.lower()}",
        )

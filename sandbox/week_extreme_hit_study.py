#!/usr/bin/env python3
"""
Study sessions where week_high / week_low are hit during overnight window.

For each scanner snapshot session:
  - Detect first touch of week_high and/or week_low in [anchor, anchor + 15h]
  - Compute price/volume context at the touch candle
  - Summarize aggregate behavior by hit type

Usage:
  python sandbox/week_extreme_hit_study.py --symbol XRPUSDT
  python sandbox/week_extreme_hit_study.py --symbol XRPUSDT --output sandbox/cache/week_extreme_study_xrp.json
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Study weekly-extreme touches per session")
    ...
    p.add_argument("--post-minutes", type=int, default=60, help="Forward window after touch")
    p.add_argument("--output", default="", help="Write full JSON study report")
    p.add_argument("--events-csv", default="", help="Optional CSV export for per-event rows")
    return p.parse_args()
...
def _first_touch(candles: list[dict[str, Any]], level: float, kind: str) -> tuple[int, dict[str, Any] | None]:
    for i, c in enumerate(candles):
        h = float(c["high"])
        l = float(c["low"])
        if kind == "week_high" and h >= level:
            return i, c
        if kind == "week_low" and l <= level:
            return i, c
    return -1, None
...
def summarize(events: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    out: dict[str, Any] = {}
    for k in ("week_high", "week_low"):
        rows = by_type.get(k, [])
        out[k] = {
            "count": len(rows),
            "avg_minutes_to_hit": round(_mean([float(r["minutes_to_hit"]) for r in rows]), 2) if rows else None,
            "median_like_minutes_to_hit": sorted([float(r["minutes_to_hit"]) for r in rows])[len(rows) // 2] if rows else None,
...
def main() -> int:
    ...
    for snap in snapshots:
        ...
        week_high = snap.get("week_high")
        week_low = snap.get("week_low")
        ...
        high_idx, high_candle = _first_touch(overnight, float(week_high), "week_high")
        low_idx, low_candle = _first_touch(overnight, float(week_low), "week_low")
        hit_h = high_idx >= 0 and high_candle is not None
        hit_l = low_idx >= 0 and low_candle is not None
    ...
    result = {
        ...
        "sessions_with_week_high_hit": high_hit_sessions,
        "sessions_with_week_low_hit": low_hit_sessions,
        "sessions_with_both_hits": both_hit_sessions,
        "week_high_hit_rate_pct": round((high_hit_sessions / session_count * 100.0), 2) if session_count else 0.0,
        "week_low_hit_rate_pct": round((low_hit_sessions / session_count * 100.0), 2) if session_count else 0.0,
    ...
    print("\nWEEKLY EXTREME TOUCH STUDY")
    print("=" * 80)
    print(f"Session: {session_id}")
    print(f"Symbol: {symbol}")
    print(f"Sessions analyzed: {session_count}")
    print(f"Week-high touched sessions: {high_hit_sessions} ({result['week_high_hit_rate_pct']:.2f}%)")
    print(f"Week-low touched sessions: {low_hit_sessions} ({result['week_low_hit_rate_pct']:.2f}%)")
    print(f"Both touched sessions: {both_hit_sessions} ({result['both_hit_rate_pct']:.2f}%)")
    print("-" * 80)
    for key in ("week_high", "week_low"):
        s = summary.get(key, {})
        print(

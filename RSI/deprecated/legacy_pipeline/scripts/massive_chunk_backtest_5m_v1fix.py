...
- Generates trades with run_simulation_trades; replays each trade on full 5m with:
    ...
    * managed: MFE ladder (5m, same-bar lock activation, cap_lock_by_mfe=True)
...
Outputs under cache/:
  ...
  V1FIX TP sweep (--tp-sweep): massive_chunk_v1fix_preflight.csv,
    massive_chunk_v1fix_tpXX_results.csv, massive_chunk_v1fix_tp_sweep_summary.csv,
    optional massive_chunk_v1fix_tpXX_ledger_<SYMBOL>.csv
"""
...
PREFLIGHT_CSV = os.path.join(CACHE_DIR, "massive_chunk_v1fix_preflight.csv")
RESULTS_CSV = os.path.join(CACHE_DIR, "massive_chunk_v1fix_results.csv")
BY_ASSET_CSV = os.path.join(CACHE_DIR, "massive_chunk_v1fix_results_by_asset.csv")
RESULTS_QUARTERLY_CSV = os.path.join(CACHE_DIR, "massive_chunk_v1fix_results_quarterly.csv")
BY_ASSET_QUARTERLY_CSV = os.path.join(CACHE_DIR, "massive_chunk_v1fix_results_by_asset_quarterly.csv")
REPORT_MD = os.path.join(_PROJECT_ROOT, "FINAL_MASSIVE_CHUNK_BACKTEST_V1FIX.md")
PREFLIGHT_V1_CSV = os.path.join(CACHE_DIR, "massive_chunk_v1fix_preflight.csv")
V1_TP_SWEEP_SUMMARY = os.path.join(CACHE_DIR, "massive_chunk_v1fix_tp_sweep_summary.csv")
...
def _paths_for_tp_v1(tp_r: float) -> tuple[str, str, str, str]:
    tag = _tp_tag(tp_r)
    results_csv = os.path.join(CACHE_DIR, f"massive_chunk_v1fix_{tag}_results.csv")
    by_asset_csv = os.path.join(CACHE_DIR, f"massive_chunk_v1fix_{tag}_results_by_asset.csv")
    results_q_csv = os.path.join(CACHE_DIR, f"massive_chunk_v1fix_{tag}_results_quarterly.csv")
    by_asset_q_csv = os.path.join(CACHE_DIR, f"massive_chunk_v1fix_{tag}_results_by_asset_quarterly.csv")
...
def build_4h_rsi(df_5m: pd.DataFrame, rsi_window: int = 20) -> pd.DataFrame:
    df_4h = (
        df_5m.resample("4h")
        .agg({"open": "first", "high": "max", "low": "min", "close": "last", "volume": "sum"})
        .dropna()
    )
    df_4h["RSI"] = RSIIndicator(close=df_4h["close"], window=rsi_window).rsi()
...
def process_symbol(
    *,
    ...
    df4h = build_4h_rsi(df5, rsi_window=rsi_window)
    if len(df4h) < sl_n + 5:
        return pd.DataFrame(), pd.DataFrame(), []
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=int,
        default=20,
        help="Wilder RSI period on 4h close (passed to RSIIndicator window).",
    )
    ...
    ap.add_argument(
        ...
        type=str,
        default="",
        help="Comma-separated replay TPs. When set, writes massive_chunk_v1fix_tp*_*.csv and tp_sweep_summary "
    ...
    if args.report_only:
        if v1_sweep:
            ...
            report_tp = os.path.join(_PROJECT_ROOT, f"FINAL_MASSIVE_CHUNK_BACKTEST_V1FIX_{_tp_tag(args.tp_r)}.md")
            write_report(
                pre,
                res,
                ba,
                ...
                title="# Massive 5m Chunk Backtest V1FIX (final)",
    ...
    if v1_sweep:
        ...
        for tp_r in tp_levels:
            print(f"=== Running TP={tp_r}R (v1fix) ===")
            ...
            if args.write_ledgers and all_ledgers:
                ...
                for sym in symbols:
                    ...
                    if not sub:
                        continue
                    path = os.path.join(CACHE_DIR, f"massive_chunk_v1fix_{tag}_ledger_{sym}.csv")
                    pd.DataFrame(sub).to_csv(path, index=False)
                    print(f"Wrote {path}")

            if not args.no_report:
                report_tp = os.path.join(_PROJECT_ROOT, f"FINAL_MASSIVE_CHUNK_BACKTEST_V1FIX_{_tp_tag(tp_r)}.md")
                write_report(
                    preflight,
                    results,
                    by_asset,
                    ...
                    strategy_lines=[
                        ...
                        f"RSI {args.rsi_l}/{args.rsi_h} (period={args.rsi_window}), SL_N={args.sl_n}, engine fee {args.fee_bps} bps RT",
                        f"Trade list from engine at TP={args.entry_tp_r}R; replay TP={tp_r}R",
                        f"MFE ladder (V1FIX): ({args.mfe1}R → {args.lock1}R), ({args.mfe2}R → {args.lock2}R); "

#!/usr/bin/env python3
"""
Run orchestrator for V1FIX massive-chunk flow (2-stage MFE ladder).

Creates a timestamped run folder under RSI/runs/, executes massive_chunk_backtest_5m_v1fix.py
with --tp-sweep, copies artifacts, and exports detailed per-trade rows (no candle data).
"""
...
from massive_chunk_backtest_5m_v1fix import (  # noqa: E402
    build_4h_rsi,
    load_merged_5m,
    parse_float_grid,
)
...
def write_replay_audit(*, trades_dir: Path, tp_levels: list[float]) -> None:
    rows: list[pd.DataFrame] = []
    for tp_r in tp_levels:
        p = trades_dir / f"trade_details_v1fix_{_tp_tag(tp_r)}.csv"
        if not p.exists():
            continue
        df = pd.read_csv(p)
        if len(df) == 0:
            continue
        df["delta_r"] = df["managed_r"] - df["baseline_r"]
        sub = df.sort_values("entry_ts_utc").head(25).copy()
        sub["audit_scope"] = "first_25_by_entry"
        rows.append(
            sub[
                [
                    "audit_scope",
        ...
        be = df[df["managed_reason"] == "BE"].copy().sort_values("entry_ts_utc").head(25)
        if len(be):
            be["delta_r"] = be["managed_r"] - be["baseline_r"]
            be["audit_scope"] = "first_25_be_exits"
            rows.append(
                be[
                    [
                        "audit_scope",
    ...
    out.to_csv(trades_dir / "replay_audit_v1fix.csv", index=False)
...
def export_trade_details(
    *,
    ...
    stages = sorted([(mfe1, lock1), (mfe2, lock2)], key=lambda x: x[0])
    ...
    for tp_r in tp_levels:
        ...
        tp_df.to_csv(out_dir / f"trade_details_v1fix_{_tp_tag(tp_r)}.csv", index=False)

    combined_df = pd.DataFrame(combined_rows).sort_values(["tp_r", "symbol", "entry_ts_utc"])
    combined_df.to_csv(out_dir / "trade_details_v1fix_all_tp.csv", index=False)


def main() -> None:
    ap = argparse.ArgumentParser(
        description="Run v1fix single-symbol/date-window sequence and package all artifacts."
    )
    ap.add_argument("--db", default=DB_PATH)
    ap.add_argument("--symbol", required=True, help="Single symbol to troubleshoot, e.g. BTCUSDT")
    ap.add_argument("--start-utc", required=True, help="Inclusive UTC start, e.g. 2025-01-01")
    ap.add_argument("--end-utc", required=True, help="Exclusive UTC end, e.g. 2025-03-01")
    ...
    run_id = f"run_v1fix_single_{symbols[0]}_{now}"
    ...
    config = {
        ...
        "script": "scripts/massive_chunk_backtest_5m_v1fix.py",
        "variant": "v1fix_single_symbol_two_stage_mfe_samebar_lock",
    }
    ...
    cmd = [
        str(py),
        str(SCRIPT_DIR / "massive_chunk_backtest_5m_v1fix.py"),
        "--db",
        args.db,
        "--symbols",
        symbols[0],
    ...
    _copy_if_exists(cache / "massive_chunk_v1fix_preflight.csv", artifacts_dir / "massive_chunk_v1fix_preflight.csv")
    _copy_if_exists(
        cache / "massive_chunk_v1fix_tp_sweep_summary.csv",
        artifacts_dir / "massive_chunk_v1fix_tp_sweep_summary.csv",
    )
    for tp_r in tp_levels:
        tag = _tp_tag(tp_r)
        for name in [
            f"massive_chunk_v1fix_{tag}_results.csv",
            f"massive_chunk_v1fix_{tag}_results_by_asset.csv",
        ]:
            _copy_if_exists(cache / name, artifacts_dir / name)
        for sym in symbols:
            led = f"massive_chunk_v1fix_{tag}_ledger_{sym}.csv"
            _copy_if_exists(cache / led, artifacts_dir / led)
        rep = PROJECT_ROOT / f"FINAL_MASSIVE_CHUNK_BACKTEST_V1FIX_{tag}.md"
    ...
    write_replay_audit(trades_dir=trades_dir, tp_levels=tp_levels)

...
Outputs (v2):
- cache/rsi_year_asset_grid_managed_v2_mfe3_tpXX.csv
- cache/rsi_year_asset_best_v2_mfe3_tpXX.csv
- cache/rsi_year_global_pattern_v2_mfe3_tpXX.csv
- FINAL_RSI_ENTRY_ADAPTIVE_PATTERN_V2_MFE3_tpXX.md
"""
...
from massive_chunk_backtest_5m_v2_mfe3 import build_4h_rsi, load_merged_5m, parse_float_grid  # noqa: E402
...
def main() -> None:
    ...
    ap.add_argument("--tp-r", type=float, default=13.0)
    ap.add_argument("--tp-sweep", type=str, default="12,13,14")
    ...
    stages = [(args.mfe1, args.lock1), (args.mfe2, args.lock2), (args.mfe3, args.lock3)]
    ...
    for tp_r in tp_levels:
        tag = _tp_tag(tp_r)
        grid_csv = os.path.join(CACHE_DIR, f"rsi_year_asset_grid_managed_v2_mfe3_{tag}.csv")
        best_csv = os.path.join(CACHE_DIR, f"rsi_year_asset_best_v2_mfe3_{tag}.csv")
        pattern_csv = os.path.join(CACHE_DIR, f"rsi_year_global_pattern_v2_mfe3_{tag}.csv")
        report_md = os.path.join(_PROJECT_ROOT, f"FINAL_RSI_ENTRY_ADAPTIVE_PATTERN_V2_MFE3_{tag}.md")
        ...
        for sym in symbols:
            ...
            if len(df5) == 0:
                ...
                continue
            print(f"{sym}: evaluating {len(combos)} RSI combos @ TP={tp_r}...")
        ...
        "",
            ...
            f"- Managed replay: 5m MFE ladder `({args.mfe1}->{args.lock1}), ({args.mfe2}->{args.lock2}), ({args.mfe3}->{args.lock3})`, TP `{tp_r}R`, cap lock by MFE",
            ...
            "## Artifacts",
            "",
            f"- `cache/{os.path.basename(grid_csv)}`",
            f"- `cache/{os.path.basename(best_csv)}`",
            f"- `cache/{os.path.basename(pattern_csv)}`",
        ]

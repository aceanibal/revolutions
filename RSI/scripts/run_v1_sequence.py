#!/usr/bin/env python3
"""
Run orchestrator for legacy V1 massive-chunk flow (2-stage MFE ladder).

Creates a timestamped run folder under RSI/runs/, executes massive_chunk_backtest_5m.py
...
from massive_chunk_backtest_5m import (  # noqa: E402
    build_4h_rsi,
    load_merged_5m,
    parse_float_grid,
)
...
def export_trade_details(
    *,
    ...
    for tp_r in tp_levels:
        ...
        tp_df.to_csv(out_dir / f"trade_details_v1_{_tp_tag(tp_r)}.csv", index=False)

    combined_df = pd.DataFrame(combined_rows).sort_values(["tp_r", "symbol", "entry_ts_utc"])
    combined_df.to_csv(out_dir / "trade_details_v1_all_tp.csv", index=False)


def main() -> None:
    ap = argparse.ArgumentParser(description="Run v1 massive-chunk sequence and package all artifacts.")
    ...
    cmd = [
        str(py),
        str(SCRIPT_DIR / "massive_chunk_backtest_5m.py"),
        "--db",
        args.db,
        "--symbols",
        ",".join(symbols),
    ...
    _copy_if_exists(cache / "massive_chunk_v1_preflight.csv", artifacts_dir / "massive_chunk_v1_preflight.csv")
    _copy_if_exists(
        cache / "massive_chunk_v1_tp_sweep_summary.csv",
        artifacts_dir / "massive_chunk_v1_tp_sweep_summary.csv",
    )
    for tp_r in tp_levels:
        tag = _tp_tag(tp_r)
        for name in [
            f"massive_chunk_v1_{tag}_results.csv",
            f"massive_chunk_v1_{tag}_results_by_asset.csv",
        ]:
            _copy_if_exists(cache / name, artifacts_dir / name)
        for sym in symbols:
            led = f"massive_chunk_v1_{tag}_ledger_{sym}.csv"
            _copy_if_exists(cache / led, artifacts_dir / led)
        rep = PROJECT_ROOT / f"FINAL_MASSIVE_CHUNK_BACKTEST_V1_{tag}.md"

...
def replay_trade_mfe_ladder_5m(
    ...
    *,
    ...
    - V1FIX uses same-bar lock activation: when MFE threshold is touched in this candle, the tighter
      stop becomes active immediately and can be hit by the same candle's adverse wick.
    ...
    for i, (ts, row) in enumerate(d.iterrows()):
        ...
        if side == 1:
            ...
            if low <= stop:
                gross = (stop - entry_price) / risk
                return ReplayResult(gross - fee_r, "BE", ts, i + 1)
        else:
            if h >= stop:
                gross = (entry_price - stop) / risk
                tag = "SL" if abs(stop - stop_init) < 1e-8 * max(1.0, abs(entry_price)) else "BE"
